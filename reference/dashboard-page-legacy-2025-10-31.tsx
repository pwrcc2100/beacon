...
import { formatPercent, getScoreStatus, SCORE_BACKGROUNDS } from '@/components/dashboard/scoreTheme';
                ...
                <TableBody>
                  {hierarchyData.currentLevel === 'team' ? (
                    ...
                    hierarchyData.data.map((agg:any, i:number)=> {
                      const renderPercentPill = (percent: number) => {
                        const status = getScoreStatus(percent);
                        return (
                          <span
                            className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold"
                            style={{ backgroundColor: status.background ?? SCORE_BACKGROUNDS.neutral, color: status.color }}
                          >
                            {Math.round(percent)}%
                          </span>
                        );
                      ...
                      const drillDownUrlBase = `/dashboard?client=${clientId}&period=${period}`;
                      let drillDownUrl = drillDownUrlBase;
                      ...
                      return (
                        <TableRow key={i} className="hover:bg-muted/50">
                          ...
                          <TableCell>{renderPercentPill(agg.wellbeing_score ?? 0)}</TableCell>
                          <TableCell>{renderPercentPill((agg.sentiment_avg ?? 0) * 20)}</TableCell>
                          <TableCell>{renderPercentPill((agg.clarity_avg ?? 0) * 20)}</TableCell>
                          <TableCell>{renderPercentPill((agg.workload_avg ?? 0) * 20)}</TableCell>
                          <TableCell>{renderPercentPill((agg.safety_avg ?? 0) * 20)}</TableCell>
                          <TableCell>{renderPercentPill((agg.leadership_avg ?? 0) * 20)}</TableCell>

  ...
  return (
    <div className="flex flex-wrap items-center gap-4">
      <div className="flex items-center gap-2">
        <label className="text-sm font-medium text-bi-textMuted whitespace-nowrap">
          {level1Label}
        </label>
        <select
          value={currentLevel1Id ?? 'all'}
          onChange={(e) => updateFilter('division_id', e.target.value === 'all' ? null : e.target.value)}
          className="min-w-[140px] rounded-bi-md border border-bi-border bg-bi-surface px-3 py-2 text-sm text-bi-text focus:outline-none focus:ring-2 focus:ring-bi-accent/40"
        >
